...
void PaintWidget::mouseReleaseEvent(QMouseEvent *event) {
    ...
    if(mode == PaintWidget::Draw)
    {
        if (type != Pen) {
            paint(image);
            Operation  op(type,penWidth,penColor,lastPoint.x(),lastPoint.y(),endPoint.x(),endPoint.y(),0, isFilling, fillColor);
            sendop(op);
        }else{
            Operation  op(type,penWidth,penColor,0,0,0,0,0, isFilling, fillColor);
            qDebug() << pointList.size() <<endl;
